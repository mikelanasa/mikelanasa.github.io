...
if (isset($_POST['firstname'])) $firstname 	= cleanse_input($_POST['firstname'], 'w');
else $firstname = NULL;
if (isset($_POST['lastname']))  $lastname 	= cleanse_input($_POST['lastname'], 'w');
else $lastname = NULL;
if (isset($_POST['email'])) 	$email 		= cleanse_input($_POST['email'], NULL);
else $email		= NULL;
if (isset($_POST['userid'])) 	$userid 	= cleanse_input($_POST['userid'], NULL);
else $userid 	= NULL;
if (isset($_POST['password'])) 	$password 	= cleanse_input($_POST['password'], NULL);
else $password 	= NULL;

// Verify Input		
if (isset($_POST['submit'])) {
	if ($firstname == NULL) $msg = errmsg($msg, 'First Name is missing');
	if ($lastname == NULL) $msg = errmsg($msg, 'Last Name is missing');
	if ($email == NULL) $msg = errmsg($msg, 'Invalid Email');
	//if (filter_var($email, FILTER_VALIDATE_EMAIL)) $msg = errmsg($msg, 'Invalid Email');
	if ($userid == NULL) $msg = errmsg($msg, 'Userid is missing');
	if ($password == NULL) $msg = errmsg($msg, 'Password is missing');
	if (strlen($password) < $minpword) $msg = errmsg($msg, "Password must be at least $minpword characters");
...
echo "<form action='$pgm' method='post'>\n
		  ...
		  <tr><td $td>&nbsp;</td>	 <td $tf><input type='submit' name='submit' value='Submit' style='background-color:lightgreen;font-weight:bold;cursor:pointer'>
											<button style='background-color: lightblue; font-weight: bold; color: black;'>
												<a href='website.php?p=logon' style='color: inherit; text-decoration: none;'>Back</a>
											</button></td></tr>\n
		  <tr><td $td>&nbsp;</td>	 <td $tf>&nbsp;</td></tr>\n
