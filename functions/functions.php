	...
	for ($i = 0; $i < $format_length; $i++) {
		...
		switch ($letter) {
			case "a":
				...
				break; // Remove any non-alphabetic characters
			case "e":
				$phrase = ltrim($phrase);
				break;						// Left Trim
			case "f":
				$phrase = ucfirst($phrase);
				break;						// Capitolize First Word
			case "i":
			...
			case "n":
				...
				break;	// Remove any non-numeric characters
			case "r":
				$phrase = rtrim($phrase);
				break;						// Right Trim
			case "s":
...
function errmsg($msg, $emsg)
{
	if ($msg == NULL) {
		...
		$msg .= '<br>' . $emsg; // Use concatenation to append the error message
	}
	return $msg;
}

// Loop options for select
function loopSelectOptions($array)
{
